...
if __name__ == "__main__":
    params=sys.argv[1]
    p=params.split('.')
    dataset =loader.load_data(stage="train",path="/projets/iris/PROJETS/lboualil/CORPUS/tweets/train_data_2015+2016+2017.csv")
    ...
    path=os.path.join("/users/iris/lboualil/TweetSumm_Model/Models/config",params)
    with open(path) as f:
        params=json.load(f)
    for train, test in kfold.split(X, Y):
        ...
        model.params['embedding_trainable'] = params['embed']  
        model.params['num_blocks'] = params['num_blocks']
        model.params['kernel_count'] = params['kernel_count']
        model.params['kernel_size'] = params['kernel_size']
        model.params['dpool_size'] = params['dpool_size']
    ...
    with open(f'/users/iris/lboualil/cross_valid_fst_cbow_all_{p[0]}.txt','w') as f:
        f.write("\n>>> Resultat mrr:  %.2f%% (+/- %.2f%%)" % (numpy.mean(mrrscores), numpy.std(mrrscores)))
        f.write("\n>>> Resultat map:  %.2f%% (+/- %.2f%%)" % (numpy.mean(mapscores), numpy.std(mapscores)))
        f.write("\n>>> Resultat ndcg@1:  %.2f%% (+/- %.2f%%)" % (numpy.mean(ndcgscores), numpy.std(ndcgscores)))
        f.write("\n>>> Resultat p@10:  %.2f%% (+/- %.2f%%)" % (numpy.mean(p_10scores), numpy.std(p_10scores)))